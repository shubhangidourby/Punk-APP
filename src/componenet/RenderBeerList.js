...
const RenderBeerList = ({ beers, toggleFavourite }) => (
  <>
    {beers.map(beer => (
      <div className="card" key={beer.id}>
        ...
        <i className={beer.isFavourite ? 'fas fa-star' : 'far fa-star'} title={beer.isFavourite ? 'Remove from Favourites' : 'Add to Favourites'} onClick={(e) => toggleFavourite(e, beer.id)} ></i>
      </div>
    ))}
  </>
);
